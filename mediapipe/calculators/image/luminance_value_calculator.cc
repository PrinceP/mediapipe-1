...
// A calculator to find luminance value.
// Input stream:
//   IMAGE: Input image frame of type ImageFrame from video stream.
// Output streams:
//   LUMINANCE_VALUE: Double for the luminance value.
    class LuminanceValueCalculator : public CalculatorBase {
    public:
        ~LuminanceValueCalculator() override = default;
...
//                             const std::vector<cv::Mat>& image_pyramid);
    ...
    REGISTER_CALCULATOR(LuminanceValueCalculator);

    ::mediapipe::Status LuminanceValueCalculator::GetContract(
    ...
    ::mediapipe::Status LuminanceValueCalculator::Open(CalculatorContext* cc) {
//        options_ =
...
//        pool_->StartWorkers();
    ...
    ::mediapipe::Status LuminanceValueCalculator::Process(CalculatorContext* cc) {
    ...
    double LuminanceValueCalculator::_getBrightness(const cv::Mat &frame) {
        ...
        for (int i=0; i < grayMat.rows; ++i){
            for (int j=0; j < grayMat.cols; ++j){
                total_intensity += (int)grayMat.at<uchar>(i, j);
            }
        }
...
//    void LuminanceValueCalculator::ComputeImagePyramid(
...
//    cv::Mat LuminanceValueCalculator::ExtractPatch(
